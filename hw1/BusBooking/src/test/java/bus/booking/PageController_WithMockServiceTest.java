...
public class PageController_WithMockServiceTest {
    @Autowired
    private MockMvc mvc;
    ...
    public void testIndex() throws Exception {
        when(tripService.getAllCities())
                .thenReturn(new HashSet<>(Arrays.asList("Lisboa", "Porto", "Coimbra", "Faro", "Braga", "Aveiro")));

        mvc.perform(get("/")).andExpect(status().isOk()).andExpect(view().name("index"))
                .andExpect(model().attribute("cities",
                        new HashSet<>(Arrays.asList("Lisboa", "Porto", "Coimbra", "Faro", "Braga", "Aveiro"))));
    }

    @Test
    public void testResultsWithEUR() throws Exception {
